...
const server = http.createServer(async (req, res) => {
  ...
  const scenario =
    url.searchParams.get('scenario') || 'valid';
  ...
  try {
    ...
    if (url.pathname === '/weather-page') {
      ...
      // This page renders values returned by the third-party service.
      // Response validation in weather.service.js should prevent invalid
      // or malicious data from reaching this downstream component.
...
<body>
  ...
  <p><strong>Condition:</strong> ${weather.condition}</p>
    ...
    // Handle invalid third-party responses gracefully.
    if (error.name === 'ThirdPartyValidationError') {
    ...
    // Handle other upstream weather-service failures.
    return sendJson(res, 502, {
...
server.listen(PORT, () => {
  console.log(
    `Weather Consumer App listening on http://localhost:${PORT}`
  );
});
